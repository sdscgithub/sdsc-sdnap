...
<?php
  ...
  if (isset($_GET['sortBy'])) {
    ...
    if (in_array("newTickets", $sortOptions, true)) {
      ...
      if (isset($_GET['reasons'])) {
        ...
        $stid_get_more_child_tickets = oci_parse($connection, "SELECT * FROM recharge_pro.invalid_charges WHERE PROJECT_NUM=3 AND INVALID_REASON IN (" . $reasonList . ")");
      } else {
        $stid = oci_parse($connection, "SELECT * FROM recharge_pro.invalid_charges WHERE PROJECT_NUM=3 AND JOB_NUM IN (" . $changesString . ")");
        $stid_get_more_child_tickets = oci_parse($connection, "SELECT * FROM recharge_pro.invalid_charges WHERE PROJECT_NUM=3");
      }
    } else {
      if (isset($_GET['reasons'])) {
      $reasonSort = $_GET['reasons'];
  ...
  if (isset($_GET['sortBy'])) {
    $sortOptions = $_GET['sortBy'];
    if ( in_array("newTickets", $sortOptions, true) ) {
      oci_execute($stid_get_more_child_tickets);
      while ( $array = oci_fetch_array($stid_get_more_child_tickets) ) {
        $ticketNum = $array["JOB_NUM"]; 
        // get master ticket number
        $stid2 = oci_parse($connection, "SELECT MRREF_TO_MR, BILLABLE FROM footprints.master3 WHERE MRID=" . $ticketNum);
        oci_execute($stid2);
        ...
        $key = substr($parent, 1); // get rid of the prefix P in front of parent ticket #s
        if ($parent != null && $parent[0] == 'P') {
          // if master of a new ticket is in there and this ticket shares the same master 
          if ( array_key_exists($key, $masterArray) && !in_array($ticketNum, $masterArray[$key]) ) {
            $masterArray[$key][] = $ticketNum;
            $ticketIndex = $array["INDX"];
            //$invalidReasons[$ticketNum][$ticketIndex][] = $array["INVALID_REASON"]; // add invalid reason to array
  ...
  foreach (array_keys($masterArray) as $value) {
    array_multisort($masterArray[$value], SORT_ASC);
  ...
  function printValues() {
    ...
    $rowNum = 0;
    foreach (array_keys($masterArray) as $value) {
      // need to grab all children tickets of this master ticket, regardless of bounced or not 
    ...
    // Begin Children Tickets
      for ($i = 0; $i < count($masterArray[$value]) - 1; $i++) {
        ...
        echo "</td>\n";
        echo "<td valign='top'>";
        if (in_array($currentTicket, $changesArray, true)) {
          echo "<i class='fa fa-star'></i>";
        }
        echo $currentTicket;
        echo "</td>\n";
      ...
      echo "</div><br>";
      $number += 1;
      //Move this rowNum post-incr into the for-loop that prints out childrenTicket data
      //$rowNum++; 
    }
