...
class Table{
  constructor(name, id, position, isUnsorted = false){
    this.name = name;
    this.id = id;
...
$(document).ready(function() {
  ...
  .catch(function(err){
    ...
    showError("Whoops, something went wrong", "Click here to reload", function(){location.reload()})
  })
  table();
...
function updateGroupPostions(){
  for(let i = 0; i < user.tables.length; i++){
    user.tables[i].position = i;
  }
}
...
function loadUsersItemsFromDB(){
  ...
  .then(function(){
    return getAllGroupsOfUser(user.databaseID);
  })
  ...
  .then(function(groups){
    ...
    .then(function(items){
      ...
      for(let i = 0; i < groups.length; i++){
        ...
        groupObj.isUnsorted = groups[i].isUnsorted == 1;
...
function createTablesFromDBandAPI(dbData, tasks){
  return createTablesFromGroups(dbData, tasks);
}
...
function createTablesFromGroups(groups, tasks){
  ...
  for(let i = 0; i < groups.length; i++){
    let group = groups[i];
    let table = new Table(group.name, group.id, i, group.isUnsorted);
  ...
  let waitForDatabase = Promise.resolve();
  if(newUnsortedTasks.length > 0){
    if(! user.hasUnsortedTable()){
      waitForDatabase = createUnsortedTable(tasks, groups);
    }else{
      let unsortedTable = user.getUnsortedTable();
      for(let i = 0; i < newUnsortedTasks.length; i++){
        unsortedTable.rows.push(newUnsortedTasks[i]);
        newUnsortedTasks[i].position = unsortedTable.rows.length;
      }
      waitForDatabase = setItems(newUnsortedTasks, unsortedTable.id);
    }
  }
...
function removeDeletedCardsFromDB(itemsFromDB, tasks){
  ...
  for(let i = 0; i < allItems.length; i++){
    ...
    if(! cardIsOpen){
      tasksToDelete.push(allItems[i]);
    }
  }

  if(tasksToDelete.length > 0){
    return deleteItems(tasksToDelete);
  }else{
    return Promise.resolve();
  }
}
...
function instantiateUser() {
  ...
  user.hasUnsortedTable = function(){
    return user.getUnsortedTable() != null;
  }

  user.hasGroupByName = function(groupName){
    for(let i = 0; i < user.tables.length; i++){
      let table = user.tables[i];
  ...
  user.getUnsortedTable = function(){
    for(let i = 0; i < user.tables.length; i++){
      let table = user.tables[i];
      if(table.isUnsorted){
        return table;
...
function setIDs() {
  return new Promise(function(resolve, reject){
    ...
    Promise.all(new Array(setIDTre, setIDZen))
    .then(function(){
