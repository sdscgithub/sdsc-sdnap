...
$(".main").on("click", "#tableTitle", function() {
  ...
  if(! user.getTableByID($groupID).isUnsorted){
    ...
    $input.keyup(function(e) {
      ...
      if (e.which === 13 || e.which === 27) {
        ...
        if (user.hasGroupByName(currentName)) {
...
function showDeleteUnsortedPrompt() {
...
function deleteTable(tableObj) {
  ...
  return Promise.resolve()
  .then(function(){
    if(! tableObj.isEmpty() && unsortedTableObj == null){
      return createUnsortedTable([],[])
      .then(function(unsortedTable){
        unsortedTableObj = unsortedTable;
        createTable(unsortedTableObj, true);
        // TODO Hide this table util it is done being populated
        return Promise.resolve();
      })
    }else{
      Promise.resolve();
  ...
  .then(function(){
    ...
    if(tableObj.rows.length > 0){
      ...
      return setItems(unsortedTableObj.rows, unsortedTableObj.id);
    }else{
      return Promise.resolve();
    }
  ...
  .then(function(){
    ...
    return deleteUserGroup(user.databaseID, tableObj);
  })

  .then(function(){
    updateGroupPostions();
    return setGroups(user.tables);
  })
