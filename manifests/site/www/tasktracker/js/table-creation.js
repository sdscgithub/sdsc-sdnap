...
function createNewTable() {
  ...
  tableObject.name = getNewTableName();
  addGroupToDB(tableObject)
  .then(function(){
...
function getNewTableName(){
  let num = 1;
  while(true){
    let newName = "New Table " + num;
    if(! (user.hasGroupByName(newName)) ){
      return newName;
    }
    num++;
  }
...
function createUnsortedTable(tasks, groups){
  let table = new Table('Unsorted', -1, user.tables.length, true);
  ...
  return addGroupToDB(table).then(function(){
    return table;
  })
}
...
function onTableUpdated(event, ui){
  ...
  for (let i = 1; i < items.length; i++) {
    ...
    task.position = i - 1;
    //TODO Maybe need to update groupID of item (which group it is in)
    newRows.push(task);
  }
  table.rows = newRows;


  updateAllItems(user.databaseID, table);
}
